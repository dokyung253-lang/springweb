...
const 개별조회 = async () => {
    ...
    // 2) 무엇을 , 쿼리스트링에 존재하는( 클릭한 게시물 ) bno에 게시물 정보를 서버에게 요청/ 응답받기
...
const 개별삭제 = async(bno) => {
    // 1) 현재 게시물 삭제하기 위해 현재게시물번호 확인(bno는 매개변수 또는 쿼리스트링) 
...
const 개별수정 = async (bno) => {
    ...
    // 2) 수정할 내용 입력받기, 테스트용
    const btitle = prompt('수정할 제목을 입력해주세요');
    const bcontent = prompt('수정할 내용을 입력해주세요');
    const bwriter = prompt('수정할 작성자를 입력해주세요');
    ...
    // 4) 결과
    if( data == true ){
        alert('수정성공'); location.reload(); // 페이지 새로고침
    }else{ alert('수정실패');}
}
