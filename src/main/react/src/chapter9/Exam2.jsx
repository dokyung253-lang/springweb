...
function WriteForm(props){
    // event / e 객체이란? 해당이벤트(onClick/onSubmit 등) 가 발생했을 떄 그 이벤트 정보를 담고있는 주체
    return(<> 
        <form onSubmit={( event )=>{ 
            event.preventDefault();  // 기존 onSubmit 이벤트 제거 // 깜빡임 제거
            console.log("이벤트객체 :" , event );
            // let gubun = document.querySelector(); [리액트는 querySelector방식 하지 않는다]
...
export default function Exam2(props){
    ...
    const 등록함수 = ( gubun, title )=> {
        if(gubun !=='' && title !== ''){
            ...
            setMessage(frmValue); 
            // 등록했을 때 setXXX() 실행되면 (상태변수를 선언한)해당 컴포넌트(Exam2)도 재호출 된다.
            // 즉] return 재실행
        }else{
            alert("빈 값 있음");
        }
