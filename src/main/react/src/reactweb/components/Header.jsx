...
export default function Header(props) {
    ...
    // [1] 로그인 상태에 따라 상단메뉴 분기 함수
    const getMyInfo = async () => {
        // 3) 헤더에 표시할 로그인된 유저아이디 가져오기
        const response = await axios.get('http://localhost:8080/api/member3/my/info', // 통신할(스프링 컨트롤러 매핑)주소
            { withCredentials : true } // * header 에 토큰(x), 쿠키(o , 토큰 포함) 전송으로 변경 *
        );
        // 4) 통신 결과 분기
        const data = response.data;
        if (data || data != false) { // 응답자료가 존재하면
    ...
    // [4] 헤더가 열리면 최초 1번 실행 , 로그인상태( 백엔드 검증해야 한다. )
    useEffect( () => {getMyInfo();}, [] ) 

    // [5] 로그아웃
    const logout = async() =>{
        // 1) axios 통신
        await axios.get('http://localhost:8080/api/member3/logout', // 통신할(스프링 컨트롤러 매핑)주소
            { withCredentials : true } // * 쿠키 전송*
        );    
        // 2) 로그인 상태 변경, 안내 후 페이지변경
        setLogin(false); // 로그아웃 했으므로 로그인 상태 변경
        alert('로그아웃 되었습니다.');
        location.href="/"; // 메인페이지 이동 
    }

    // JS 삼항연산자 , 조건 ? 참실행문 : 거짓실행문 , 조건이 참이면 참 실행문 거짓이면 거짓실행문
