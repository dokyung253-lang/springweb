...
public class BoardController {
    ...
    public boolean boardWrite(@RequestBody BoardDto boardDto ) { //@RequestParam vs @RequestBody
        return true; // dao 호출 생략하고 임의의 값으로 연습 중
    }
       ...
       public ArrayList<BoardDto> boardPrint(){
        ...
        return list; // dao 호출 생략하고 임의의 값으로 연습중
        }

    // 3. http://localhost:8080/board/detail?bno=1
        @GetMapping("/detail")
                public BoardDto boardDetail( @RequestParam int bno ){
            BoardDto boardDto = new BoardDto( 1, "내용", "작성자1" );
            return boardDto; // dao호출 생략하고 임의의 값으로 연습중
        }

    // 4. http://localhost:8080/board/detail?bno=1
        @DeleteMapping
                public boolean boardDelete( @RequestParam int bno ){
            return false; // dao 호출 생략하고 임의의 값으로 연습중
        }
    // 5. http://localhost:8080/board/detail?bno=1
            @PutMapping
                    public boolean boardUpdate( @RequestParam BoardDto boardDto ) {
                return true; // dao호출 생략하고 임의의 값으로 연습중
