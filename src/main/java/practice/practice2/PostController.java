...
// @Controller // *(2) HTTP기능(GET/POST/PUT/DELETE)
// @ResponseBody // *(3) HTTP 응답 Content-Type 자동 설정
@RestController // @Controller(+@Component) + @ResponseBody // 빈 등록(싱글톤 대응) + HTTP 통신기능 포함(View화면 반환) + ResponseBody 포함(값 JSON에 반환)
...
public class PostController {
    ...
    메소드 란? : 상호작용(주고받는 쌍방향 행동)
        매개변수? 매개(중개)변수(정해져있지 않은 값) : 메소드가 호출될 때 들어오는 값(인수)
        반환값? 메소드가 종료될 때 호출했던 곳으로 반환해주는 값 (리턴값)
     HTTP 란? : 상호작용 (request / response)
    ...
    */
    ...
    @PostMapping // 추가할 url 없으면 이렇게 비워놔도 된다.
    ...
    // Map 이란? key와 value 한 쌍으로 엔트리 갖고 여러개 엔트리 저장한다. <---> DTO/JSON
    ...
    // 4. 개별글 수정 PUT http://localhost:8080/practice2/post
    ...
    // 5. 개별글 삭제 DELETE http://localhost:8080/practice2/post
