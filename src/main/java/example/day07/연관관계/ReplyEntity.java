...
public class ReplyEntity {
    ...
    private BoardEnrtity boardEntity;
}
/*
    - 영속성이란?  자바객체와 데이터베이스 데이터 간의 매핑/연결 상태
    ...
    - cascade 속성이란? PK와 FK 제약조건 옵션
        ...
        CascadeType.DETACH : 부모가 영속 해제하면 자식도 같이 영속 해제된다.
        CascadeType.REFRESH : 부모가 재호출(갱신)되면 자식도 같이 재호출(갱신) 한다.
        CascadeType.PERSIST : 부모가 저장 되면 자식도 같이 저장된다.

    - fetch : 부모[PK]조회 시, 자식[FK] 관계에서 엔티티 조회여부 선택
        FetchType.EAGER : (기본값) 해당 엔티티 조회시 참조 엔티티 모두 즉시 조회한다.
            - 특징 : 초기 로딩 느리다. 재사용 시 빠르다. *불필요한 정보까지 있을 경우 성능 저하*
        FetchType.LAZY : 해당 엔티티 조회 시 참조 엔티티는 조회하지 않는다. < 참조 엔티티 호출 시 조회 >
   ...
   - 단방향 / 양방향 활용
      ...
      - 만약에 3번 게시물에 댓글을 등록한다면 ** fk 필드에 fk값인 3 대신에 3을 갖는 fk엔티티 찾아서 대입하자 **
