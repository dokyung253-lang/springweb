...
public class JWTService {
    ...
    public String 토큰생성 (String data){
        String token = Jwts.builder() // [s] 토큰 생성 시작
                ...
                .claim("data", data )
                .setIssuedAt( new Date() ) //토큰 발급날짜/시간, new Date() : 시스템날짜/시간
...
/*
    JWT json web token(징표)
        1. 정의 : JSON 형식의 데이터를 사용하기 위한 토큰기반의 인증 형식
        2. 목적 : 웹/앱에서 인증과 권한부여/확인 사용(클라이언트) vs 세션(서버)
        사용법
            설치 :
