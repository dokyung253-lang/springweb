...
/*
    ...
    2] Optional< > : 객체 내 null 값 제어 기능/함수 제공하는 클래스
       목적 : null에 따른 안전한 객체 사용
    ...
    3] JPA CPUD 기본 제공
        ...
        4. .deleteById(삭제할 PK번호)     : 특정 pk번호의 엔티티 삭제(delete), 반환타입 : void
        5. 수정함수는 존재하지 않는다.
            < 영속성 특징 >
                ...
                * 영속성이란? 데이터베이스와 자바객체 연결하는 관계
                ...
                - @Transactional 갖는 클래스/메소드는 여러 SQL들을 하나의 묶음으로 한 번에 처리한다.
                    - 즉] 트렌젝션이란? 여러 SQL들을 묶어서 하나라도 실패하면 모두 실패(Rollback) 모두 성공이면 최종성공(Commit)
                    - 예 ] 이체 기능은 [1] 입금 [2] 출금 2개 이상의 기능을 묶음 기능
                        - 입금과 출금 중에 하나라도 문제가 발생하면 전체(이체) 휘소
