...
/*
    ...
    2. 다운로드 : 서버가 클라이언트에게 데이터 파일을 전송하는 행위
    3. 스트림 : 데이터가 이동하는 흐름, 스트림API / CSV, 엑셀 파일처리 / 네트워크 등
    4. 버퍼 : 데이터가 이동하는 흐름간의 처리속도를 일정하게 하기 위한 임시 메모리 * 데이터가 이동하는 흐름하는 순간에 순간 저장소 = 버퍼 *
    5. 자바입출력 클래스
        1) FileInputStream
...
/*
    (1) 쿼리스트링 , URL ? 변수명 = 값
    (2) 본문 타입 , URL , {"변수명" : 값 }, content-Type : application/Json
