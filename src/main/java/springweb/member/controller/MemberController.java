...
public class MemberController {
    ...
    public ResponseEntity<?> login(@RequestBody MemberDto loginDto, HttpSession session){
        ...
        if(result) {
            ...
            // 2) 로그인 성공한 회원의 아이디를 세션객체 내 저장, .setAttribute( "속성명" , 속성값 );
    ...
    // [4] 마이페이지 == GET == 현재로그인된 회원정보 == 세션 저장된 정보로 조회
    @GetMapping("/my/info")
    public ResponseEntity<?> myInfo( HttpSession session ){
        ...
        // 3) 로그인 상태이면, Object --> String 로 다운캐스팅
        String loginMid = (String)obj;
        // 4) 로그인된 mid로 서비스에게 번달하여 로그인된 mid의 dto 받아오기
