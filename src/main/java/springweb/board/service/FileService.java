...
public class FileService {
    ...
    // 업로드 경로 1] 로컬 환경
    private String baseDir = System.getProperty("user.dir"); // 현재 프로젝트명 , C:\Users\임도경\OneDrive\바탕 화면\springweb
    ...
    public String upload(MultipartFile uploadFile) {
        ...
        // 2) 업로드할 파일의 경로 잡아줌 * 서버의 전체 경로 * ,   개발자(src파일) -- 배포/실행--> 서버(build파일) <-- 클라이언트(사용자) (2번은 선택)
        File uploadPath = new File(uploadDir); // 업로드할 UploadDir+파일명을 file 객체 내 대입
        // ** 만약에 해당 경로의 폴더가 존재하지 않으면 폴더 생성
        if (uploadPath.exists() == false) {// !: 부정문 , : file객체.exist() :경로가 존재하면 false
            uploadPath.mkdirs(); //file객체.mkdir() : 경로/폴더 생성
        }
        // 3) 업로드, 실제업로드경로 + 파일명
        // ** 만약에 서로 다른 사람/요청이 동일한 파일명으로 다른 파일 업로드하면 고유식별(무결성) 깨짐
        ...
        String uuid = UUID.randomUUID().toString(); // UUID이란? 중복없는 난수 문자열 생성 함수( 고유성 보장 )
            // * UUID _ 파일명 : UUID와 파일명 사이에 _언더바 구분하자 (왜? 추후에 분리해야 하므로)
            // * UUID에는 _언더바가 절대없다. 하지만 파일명에는 _언더바 존재할 수 있다.( 파일명 치환 )
        ...
        try {
            uploadFile.transferTo(uploadRealPath); // 업로드파일을 특정한 경로에 전달 * 예외처리 항상 발생 *
            return fileName; // 업로드된 파일은 DB에 저장한다.
        } catch (IOException e) {
