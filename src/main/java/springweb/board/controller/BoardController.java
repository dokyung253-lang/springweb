...
public class BoardController {
    ...
    // [1-2] 회원제 글 등록 + 토큰 정보
    @PostMapping("/write2")
    public ResponseEntity<?> write2(
        @RequestBody BoardDto boardDto,
                @RequestHeader ("Authorization") String token){
        // 1] 매개변수로 jwt 토큰 받는다.
        // 2] 만약에 토큰이 없거나 Bearer 시작하지 않으면, 문자열.startWith("시작문자") : 문자열내 시작문자가 존재하면 true
    ...
    // [1-3] 회원제 글 등록 + 토큰 정보 + 첨부파일 ( content-Type: multipart/form-data 변경)
    @PostMapping("/write3")
